...
export const getServerSideProps = async () => {
    ...
    const pageIntro = {
        ...
        description: "Chúng tôi tổ chức các hoạt động đa dạng để kết nối cộng đồng và lan tỏa tình yêu thiên nhiên. Từ những chuyến hiking khám phá núi non, camping dưới bầu trời đầy sao, đến các workshop giáo dục về bảo vệ môi trường.",
    };
    ...
    const scheduleItems: ActivityScheduleItem[] = [
        {
            ...
            description: "Chuyến leo núi Takao - ngọn núi gần Tokyo, phù hợp cho người mới bắt đầu và gia đình.",
        ...
        },
        {
            ...
            description: "Hoạt động cắm trại bên hồ Kawaguchi, trải nghiệm cuộc sống ngoài trời và học hỏi kỹ năng sinh tồn.",
        ...
        },
        {
            ...
            description: "Tổ chức workshop giáo dục về bảo vệ môi trường và phát triển bền vững cho cộng đồng.",
    ...
    const scheduleSection: ActivityScheduleSection = {
        sectionIntro: {
            ...
            subtitle: "Tham gia các hoạt động sắp diễn ra của Bàn Chân Xanh",
        },
        scheduleItems,
    };

    const joinSection: JoinActivitySection = {
        sectionIntro: {
            tag: "Tham gia cùng chúng tôi",
            title: "Đăng ký tham gia hoạt động",
            subtitle: "Bạn muốn tham gia các hoạt động của Bàn Chân Xanh? Hãy đăng ký ngay để nhận thông báo về các sự kiện sắp tới và cơ hội tham gia cùng cộng đồng.",
        },
    ...
    return (
        <Layout data={globalData ? convertGlobalInfoToLayoutData(globalData) : props.layout.data}>
                    ...
                    <div className="row justify-content-center">
                        <div className="col-lg-8">
                            <div className="wpo-section-title text-center">
                                <span>{(activityContent?.pageIntro || props.activityContent.pageIntro).tag}</span>
                                <h2>{(activityContent?.pageIntro || props.activityContent.pageIntro).title}</h2>
                                <p>{(activityContent?.pageIntro || props.activityContent.pageIntro).description}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section className="wpo-activities-section section-padding">
                <div className="container">
                    <div className="row">
                        {(activityContent?.activityItems || props.activityContent.activityItems).map((activity, index) => {
                            const imageSrc = typeof activity.image === 'string'
                                ? activity.image
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-lg-12">
                            <div className="wpo-section-title text-center">
                                <span>{(activityContent?.scheduleSection || props.activityContent.scheduleSection).sectionIntro.tag}</span>
                                <h2>{(activityContent?.scheduleSection || props.activityContent.scheduleSection).sectionIntro.title}</h2>
                                <p>{(activityContent?.scheduleSection || props.activityContent.scheduleSection).sectionIntro.subtitle}</p>
                            </div>
                        </div>
                    </div>
                    <div className="row">
                        <div className="col-lg-12">
                            <div className="wpo-schedule-wrap">
                                {(activityContent?.scheduleSection || props.activityContent.scheduleSection).scheduleItems.map((schedule, index) => (
                                    <div key={index} className="schedule-card">
                                        ...
                                        <div className="schedule-button">
                                            <a href={schedule.buttonLink} className="theme-btn">
                                                {schedule.buttonText}
                                            </a>
                                        </div>
                                    </div>
                            ...
                            <div className="join-section">
                                <div className="join-content">
                                    <span className="join-tag">
                                        {(activityContent?.joinSection || props.activityContent.joinSection).sectionIntro.tag}
                                    </span>
                                    <h2 className="join-title">
                                        {(activityContent?.joinSection || props.activityContent.joinSection).sectionIntro.title}
                                    </h2>
                                    <p className="join-description">
                                        {(activityContent?.joinSection || props.activityContent.joinSection).sectionIntro.subtitle}
                                    </p>
                                </div>
                                <div className="join-button">
                                    <a href={(activityContent?.joinSection || props.activityContent.joinSection).buttonLink} className="theme-btn">
                                        {(activityContent?.joinSection || props.activityContent.joinSection).buttonText}
                                    </a>
                                </div>
                            </div>
                        </div>
